...
int main(int argc, char **argv, char **env)
{
  ...
  if (!missFile.is_open())
  {
    std::cout << "missFile is not open" << std::endl;
  }
  ...
  for (simcyc = 0; simcyc < MAX_SIM_CYC; simcyc++)
  {
    ...
    if (currentlyReading)
    {
      ...
      if (cntReading % 5 == 2)
      {
        pdfFile << top->a0 << std::endl;
      }
    }

    if (top->hit_miss_indicator == 1)
    {
      missFile << "HIT" << std::endl;
    }
    else if (top->hit_miss_indicator == 2)
    {
      missFile << "MISS" << std::endl;
    }
